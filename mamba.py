...
# Config
# =========================
DATA_DIR    = "./data_train0"
DATA_DIR1   = "./data_train1"
...
def time_since_last_status(s, target):
    ...
    target: 0 or 1
    return: số bước liên tiếp gần nhất bằng target (tính đến thời điểm hiện tại)
    """
    T = len(s)
...
class DKF1_Variational(layers.Layer):
    """
    Variational 1-D sequence denoiser (giữ nguyên như cũ, hiện đang comment trong model).
    """
...
# ====== NEW: encode time-since-on/off once, rồi share cho air + water ======
ctx_ts_on  = enc_block(inp_ton,  "ts_on")
ctx_ts_off = enc_block(inp_toff, "ts_off")

# Base context cho nhánh air
...
model_type  = 'NNiSS_residual'

if model_type == 'NNrSS_residual':
...
elif model_type == 'NNiSS_residual':
