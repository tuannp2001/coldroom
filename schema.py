...
def compute_off_segment_stats(signal: np.ndarray,
    ...
    Trả về:
        means : [m1, m2, ..., mK]  (mean mỗi OFF segment)
        vars  : [v1, v2, ..., vK]  (variance mỗi OFF segment)
    """
...
# =========================
# 3. Align 2 series theo chiều dài
# =========================
def align_series(a: np.ndarray, b: np.ndarray, mode: str = "truncate_min"):
    """
    Đưa 2 chuỗi về cùng length để so sánh / vẽ.
    mode:
        - "truncate_min": cắt cả 2 về min(len(a), len(b))  (an toàn, không invent thêm data)
        - "pad_last": pad chuỗi ngắn bằng giá trị cuối cùng
    """
...
def upsample_variance(vars_smooth: np.ndarray, target_len: int) -> np.ndarray:
    """
    Upsample 1D array vars_smooth lên độ dài target_len bằng nội suy tuyến tính.

    Parameters
    ----------
    vars_smooth : np.ndarray, shape (K,)
        Chuỗi variance (hoặc trend variance) đã được làm mượt.
    target_len : int
        Độ dài mong muốn (ví dụ N = số điểm của time series gốc).

    Returns
    -------
    upsampled : np.ndarray, shape (target_len,)
        Phiên bản nội suy tuyến tính của vars_smooth.
    """
...
# ... phần trên giữ nguyên: load_run, compute_off_segment_stats ...

if __name__ == "__main__":
    # ---- Load 2 run ----
    Ta0, Tw0, To0, Ts0, Tr0, status0 = load_run("data_train0")
    Ta1, Tw1, To1, Ts1, Tr1, status1 = load_run("data_train1")
    Ta_val, Tw_val, To_val, Ts_val, Tr_val, status_val = load_run("data_validation")
    ...
    # ---- Tính mean & variance cho từng OFF segment ----
    ...
    # =========================
    # 1) Plot variance, giữ nguyên độ dài mỗi profile
    # =========================
    ...
    # =========================
    # 2) Plot mean, giữ nguyên độ dài mỗi profile
    # =========================
