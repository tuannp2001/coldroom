...
# ---- DKF1_Variational (giữ nguyên như training, nếu model không dùng thì cũng không sao) ----
...
# Nếu trong training bạn đã dùng PhysicalLossLayer, cần định nghĩa lại (để load được):
...
class PhysicalLossLayer(layers.Layer):
    ...
    def call(self, inputs, training=None):
        # training=False trong inference nên add_loss cũng không quan trọng
        # Giả sử inputs = [dTa_pred, dTw_pred, twmt_last, tsmt_last, tamt_last, gate]
...
for t in range(WARM_STEPS-1, N-1):
    ...
    # Predictions: model PINN trả dTa, dTw
